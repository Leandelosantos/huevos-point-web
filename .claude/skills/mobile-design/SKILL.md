...
**Este proyecto es web (no app nativa)**. El skill aplica a diseño responsive móvil de la SPA.
**Stack:** React 18 + Vite + Tailwind CSS + GSAP 3. Breakpoints Tailwind estándar.
...
### Adaptaciones mobile específicas por sección

| Sección | Mobile (< 768px) | Desktop (> 1024px) |
|---------|-------------------|---------------------|
| **Hero** | Título 40–56px, imagen estática (sin video), CTA full-width | Título 80–120px, video full quality, parallax multi-capa |
| **Story** | Scroll vertical simple, imagen arriba + texto abajo | Pin horizontal con 3 slides + parallax interno |
| **Products** | Stack vertical, 1 card por fila, tap para expandir | Grid asimétrico 60/40, hover interactions |
| **Process** | Stepper vertical simple con scroll reveal, sin pin | Pin completo con scroll distance extendida |
| **Marquee** | Velocidad reducida, font size menor | Velocidad estándar + hover pause |
| **Navbar** | Hamburger menu → full-screen overlay | Links horizontales visibles |

### Touch interactions en ProductCard

- **Tap (1er tap):** Revela descripción completa + botón CTA (equivalente al hover de desktop)
- **Tap (2do tap / botón):** Ejecuta CTA (agregar al pedido)
- **No hover states en mobile** — usar `@media (hover: hover)` para aislar hover de desktop

### Reglas GSAP en mobile (SRS §6.4.2)

- Story section: scroll vertical simple (sin horizontal pin). Sin `pin: true` en mobile.
- Process section: stepper vertical. Sin pin completo.
- Parallax multi-capa: solo desktop (>1024px). Mobile = fade reveals simples.
- `prefers-reduced-motion`: todos los efectos se eliminan, elementos aparecen directamente.
- `ScrollTrigger.matchMedia()` obligatorio para divergencia desktop/mobile.

### Touch targets

- Táctiles mínimo **44×44px** (iOS) / **48×48px** (Android).
- CTA principal ("Hacer pedido") en zona de pulgar (bottom of viewport en mobile).
- Espaciado mínimo entre targets táctiles: 8px.

### Swipe en Story (mobile)

La Story section en mobile implementa swipe horizontal entre los 3 paneles (Origen → Selección → Tu mesa). Alternativa al pin horizontal de desktop.
