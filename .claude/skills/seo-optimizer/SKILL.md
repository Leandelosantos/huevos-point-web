...
**Tipo de sitio:** SPA (Single Page Application) con una sola ruta. Prerendering estático via `vite-plugin-prerender`.
**URL canónica:** `https://huevospoint.com.ar`
**Idioma/locale:** Español Argentina (`es_AR`)
**Negocio:** Venta de huevos premium a domicilio. LocalBusiness en Buenos Aires, Argentina.

### Meta tags definidos (SRS §6.2.1)

```html
<title>Huevos Point — Huevos Premium de Campo Libre | Entrega a Domicilio</title>
<meta name="description" content="Huevos seleccionados de granjas sustentables. Campo libre, orgánicos y especiales. Pedí online con entrega en el día.">
...
```json
{
  ...
  "priceRange": "$$",
  "hasOfferCatalog": { "@type": "OfferCatalog", "name": "Catálogo de Huevos" }
}
```

### Keywords objetivo (mercado AR)

Primarias: `huevos de campo libre`, `huevos orgánicos Buenos Aires`, `huevos premium entrega domicilio`
Long-tail: `comprar maple de huevos`, `huevos pastoriles omega-3`, `huevos doble yema Buenos Aires`

### Desafíos SEO específicos de esta SPA

- **SPA + SEO:** Usar `vite-plugin-prerender` para generar HTML estático en build. Al ser 1 sola ruta, cubre el 100% del contenido indexable.
- **Core Web Vitals críticos:** LCP < 2.5s (hero image/video), CLS < 0.1 (reservar dimensiones en imágenes), TBT < 200ms (JS < 150KB gz)
- **GSAP y CLS:** Las animaciones GSAP que mueven elementos NO deben causar layout shift antes de que el JS cargue. Aplicar `visibility: hidden` inicial y revelar en JS.
- **Imágenes:** WebP + AVIF, servidas desde Supabase Storage CDN. `loading="lazy"` para below-fold. Dimensiones siempre declaradas.
- **Fonts:** WOFF2 self-hosted, `font-display: swap`. Preload de Playfair Display 700 y Satoshi 400.
- **robots.txt:** Permitir crawling completo. Bloquear `/api/` si se implementa Express.

### Objetivos de conversión (KPIs del SRS §1.2)

| Objetivo | Meta |
|----------|------|
| Conversion rate (formularios/visitantes) | > 5% |
| Usuarios que llegan a sección productos | > 65% |
| Duración promedio de sesión | > 90 segundos |
| CTR WhatsApp/teléfono | > 8% |
| Bounce rate | < 35% |
