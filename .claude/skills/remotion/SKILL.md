...
**Uso de Remotion en este proyecto:** Creación de videos promocionales para redes sociales (Instagram Reels, WhatsApp Status) y assets de video para el hero de la landing page.
...
**Tipografía:**
- Display/Hero: **Playfair Display** 700/900 → usar `@remotion/google-fonts` o archivo WOFF2
...
```typescript
// Video hero (loop para website)
// Duración: 8s · 30fps · 1920×1080
// Concepto: huevos sobre superficie de madera oscura con iluminación de estudio
// Animación: zoom out muy lento (Ken Burns) + grain overlay animado

// Reel Instagram (vertical)
// Duración: 15s · 30fps · 1080×1920
// Concepto: Preloader counter 0→100% → reveal producto → precio en JetBrains Mono

// Story promo (cuadrado)
// Duración: 6s · 30fps · 1080×1080
// Concepto: nombre producto (Playfair Display) + precio + marquee "HUEVOS POINT • PREMIUM EGGS •"
```

### Textos del marquee

```
"HUEVOS POINT • PREMIUM EGGS • FARM TO TABLE • CALIDAD SUPERIOR •"
```

### Animaciones frame-based coherentes con GSAP del sitio

- Ease preferido: `Easing.bezier(0.6, 0.01, -0.05, 0.95)` (equivalente a `power3.out` de GSAP)
- Reveal de texto: interpolate opacity 0→1 + translateY 40→0 en 15 frames
- Stagger entre elementos: 5 frames entre cada uno
- Preloader counter: interpolate lineal 0→100 en `2.5 * fps` frames
