...
**Tipo de deploy:** SPA estática (Vite build → `dist/`) + Supabase BaaS. Sin servidor propio ni Kubernetes.
**Stack de deploy:** Vercel o Netlify (auto-deploy desde `main`). NO Docker, NO Kubernetes para la SPA.
...
# 2. test (vitest)
# 3. build (vite build)
# 4. deploy (vercel --prod o netlify deploy)
```

### Variables de entorno en CI/CD

```env
VITE_SUPABASE_URL        # En Vercel/Netlify env vars (no en código)
VITE_SUPABASE_ANON_KEY   # En Vercel/Netlify env vars
VITE_SITE_URL            # https://huevospoint.com.ar
VITE_WHATSAPP_NUMBER     # En Vercel/Netlify env vars
# service_role_key → SOLO en Supabase Dashboard Edge Function secrets
```
...
Migraciones en `supabase/migrations/`. **Nunca modificar producción manualmente.**

### Deployment checklist

- [ ] `npm run build` sin errores TypeScript
- [ ] `vite-plugin-prerender` genera HTML estático de la SPA
- [ ] `.env.local` NO incluido en el deploy (está en `.gitignore`)
- [ ] `.env.example` en repo con keys vacías
- [ ] Auto-deploy desde branch `main`
- [ ] Variables de entorno configuradas en Vercel/Netlify
- [ ] Supabase RLS policies verificadas antes del go-live
- [ ] GSAP license verificada para uso comercial
- [ ] SSL activo (automático en Vercel/Netlify)
- [ ] OG image (`/public/og-image.jpg` 1200×630) presente
...
- Errores runtime: Sentry (opcional, Error boundary React)
- DB: Supabase Dashboard (queries, logs, RLS)
